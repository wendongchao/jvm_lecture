 ...
 * @date: created in 2020/1/15 23:18
 * @modified by:
 */
 ...
 *
 * 线程上下文类加载器的重要性：
 * SPI（service Provider Interface）: 服务提供接口（由服务制定者提供接口，由服务的实现着实现接口）
 ...
 *
 * 线程上下文类加载器就是当前线程的Current ClassLoader（当前类加载器）
 * 在双亲委托模型下，类加载时有下至上的，即下层的类加载器会委托上层进行加载，但是对于SPI来说，有些接口时java核心库所提供的，
 * 而java核心库是由启动类加载器来加载的，而这些接口的实现却来自不同的jar包（厂商提供），java的启动类加载器是不会加载其他
...
public class MyTest24 {
    public static void main(String[] args) {
         ...
         * null
         * thread类的加载器是启动类加载器，thread上下文加载器是系统类加载器
         */
